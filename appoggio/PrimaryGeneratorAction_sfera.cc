...
#include <iostream>
#include <fstream>
G4int dim=100000;//100000;//5000000;
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
...
: G4VUserPrimaryGeneratorAction(), fParticleGun(0), fDetector(det)
{
    ...
    std::string line;
    std::ifstream myfile
    ("/Users/annamaria/lavoro/X17/NeutronSource_e+e-/e_0.347_V_17.0_0.00256.txt");//e_0.347_EM.txt");//e_0.347_V_17.0_0.00256.txt");//_5m.txt
...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
    ...
    G4double Rmax = fDetector->GetContainerRadius()-fDetector->GetContainThickness()-0.02*mm;//  3*cm;//
    G4double Rmax2 = Rmax*Rmax;
    G4double R2 = G4UniformRand()*Rmax2;
    G4double R_rnd  = std::sqrt(R2);
    
    G4double cosTheta1 = 2*G4UniformRand() - 1;  //cosTheta uniform in [0, pi]
    G4double sinTheta1 = std::sqrt(1. - cosTheta1*cosTheta1);
    G4double phi1      = twopi*G4UniformRand();  //phi uniform in [0, 2pi]
    G4ThreeVector ur(R_rnd*sinTheta1*std::cos(phi1),R_rnd*sinTheta1*std::sin(phi1),R_rnd*cosTheta1);
